...
NOMBRE_ARCHIVO = 'ventas.json' #constante
def leer_archivo(): #funcion de archivo
    if os.path.exists(NOMBRE_ARCHIVO):
        ...
        return []
def guardar_archivo(datos): #funcion de archivo
    archivo = open(NOMBRE_ARCHIVO, 'wt', encoding='UTF-8')
    json.dump(datos, archivo, ensure_ascii=False, indent=2)
    archivo.close()

def pedir_numero(mensaje): #validaciones
    while True:
        valor = input(mensaje)
        if valor.isnumeric():
            return int(valor)
        else:
            print('No ingresó un número, intente de nuevo.')
...
def proximo_id(ventas): #funciones principales
    if len(ventas) == 0:
        return 1
    id_mas_alto = 0
    for venta in ventas:
